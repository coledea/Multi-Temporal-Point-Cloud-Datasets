...
parser = argparse.ArgumentParser(prog='Belvedere Glacier - Dataset Statistics Computation')
...
def compute_statistics(input_folder, output_log_path, leave_progress_bar=False):
	...
	for epoch in os.scandir(input_folder):
		if not os.path.isdir(epoch.path):
			continue
		processing_order[0].append([])
		for parts in os.scandir(epoch.path):
			if not parts.name.endswith('.laz'):
				continue
			processing_order[0][-1].append(parts.path)
