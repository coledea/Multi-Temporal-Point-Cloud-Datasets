...
parser = argparse.ArgumentParser(prog='Mariakerke Bad - Dataset Statistics Computation')
...
def compute_statistics(input_folder, output_log_path, leave_progress_bar=False):
	...
	epochs = []
	for months in os.scandir(os.path.join(input_folder, 'scans')):
		for day in os.scandir(months.path):
			for scan in os.scandir(day.path):
				if scan.name.endswith('.laz') and scan.name != '171227_145945_tr.laz':    # exclude corrupted file
