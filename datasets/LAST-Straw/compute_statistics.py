...
parser = argparse.ArgumentParser(prog='LAST-Straw - Dataset Statistics Computation')
...
def compute_statistics(input_folder, output_log_path, leave_progress_bar=False):
	...
	for subfolder in os.scandir(input_folder):
		if not 'variety' in subfolder.name:
			continue
		for scan in os.scandir(subfolder.path):
			scene_name = scan.name.split('_')[0]
