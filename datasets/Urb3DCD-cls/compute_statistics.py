...
parser = argparse.ArgumentParser(prog='Urb3DCD-cls - Dataset Statistics Computation')
...
def compute_statistics(input_folder, output_log_path, leave_progress_bar=False):
	...
	for split in os.scandir(input_folder):
		if not os.path.isdir(split.path):
			continue
		for change_type in os.scandir(split.path):
			if not os.path.isdir(split.path):
				continue
			for scene in os.scandir(change_type.path):
				if not os.path.isdir(scene.path):
					continue
				for epoch in os.scandir(scene.path):
					processing_order.append(epoch.path)
	...
	for scan in tqdm(processing_order, leave=leave_progress_bar):
		...
		if scan.endswith('PC1.ply'):
			changes = pointcloud_raw['params']['pred']
