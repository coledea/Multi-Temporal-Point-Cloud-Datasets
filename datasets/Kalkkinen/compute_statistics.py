...
parser = argparse.ArgumentParser(prog='Kalkkinen - Dataset Statistics Computation')
...
def compute_statistics(input_folder, output_log_path, leave_progress_bar=False):
	...
	epochs = []
	for epoch in ['kalkkinen2000pc', 'kalkkinen2021pc']:
		for scan in os.scandir(os.path.join(input_folder, epoch)):
	...
	compute_dataset_statistics([epochs], 
							...
							output_log_path, 
							leave_progress_bar,
							position_offset=[-3434050, -6796650, 0],
							txt_delimiter=',',
							txt_has_header=True)
