...
parser = argparse.ArgumentParser(prog='Trail Valley Creek - Dataset Statistics Computation')
...
def compute_statistics(input_folder, output_log_path, leave_progress_bar=False):
	...
	epochs = [[epoch.path] for epoch in os.scandir(input_folder) if epoch.name.startswith('PermaSAR_TLS')]
	config = EvaluationConfig(statistics_to_compute=[Statistics.NUM_POINTS, Statistics.AVG_DISTANCE],
						   output_log_path=output_log_path,
						   leave_progress_bar=leave_progress_bar,
						   position_offset=[-560897, -7626183, 0],
						   txt_has_header=True)
