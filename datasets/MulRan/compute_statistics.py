...
parser = argparse.ArgumentParser(prog='MulRan - Dataset Statistics Computation')
...
def compute_statistics(input_folder, output_log_path, leave_progress_bar=False):
	...
	for entry in os.scandir(input_folder):
		if not entry.is_dir():
			continue
		scan_path = os.path.join(entry.path, 'sick_pointcloud.las')
		if os.path.exists(scan_path):
			scene_name = entry.name[:-2]
			if scene_name in scenes:
				scenes[scene_name].append([scan_path])
			else:
				scenes[scene_name] = [[scan_path]]
