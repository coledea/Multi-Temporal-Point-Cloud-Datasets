...
parser = argparse.ArgumentParser(prog='Eschikon - Dataset Statistics Computation')
parser.add_argument('input_path', help='The folder of the exported point clouds')
...
def compute_statistics(input_folder, output_log_path, leave_progress_bar=False):
	...
	processing_order = []
	for box in os.scandir(input_folder):
		if not box.is_dir():
			continue
		processing_order.append([])
		for epoch in os.scandir(box.path):
