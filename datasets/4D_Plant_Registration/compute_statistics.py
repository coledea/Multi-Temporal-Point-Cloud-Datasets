...
parser = argparse.ArgumentParser(prog='4D Plant Registration - Dataset Statistics Computation')
...
def compute_statistics(input_folder, output_log_path, leave_progress_bar=False):
	...
	for plant_type in os.scandir(input_folder):
		if not plant_type.is_dir():
			continue
		for plant in os.scandir(plant_type.path):
			processing_order.append([])
			for epoch in os.scandir(plant.path):
